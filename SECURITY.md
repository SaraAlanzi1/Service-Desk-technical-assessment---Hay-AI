...
- **Confirmed via production bug report**: Firebase Storage has never been provisioned in the production project (`firebase deploy --only storage` still fails with "Firebase Storage has not been set up... click 'Get Started'"). Attaching a photo to a New Ticket submission previously hung on "Submitting..." indefinitely instead of showing an error — diagnosed via a Node-side reproduction (clean, immediate `storage/unknown` 404 rejection outside the browser) versus the in-browser symptom (an opaque CORS/network-layer failure against a nonexistent bucket, which the Storage SDK's own retry-with-backoff treats as transient rather than terminal, so it never rejects in a bounded time). Fixed the *symptom* — the form now wraps every upload call in a 20s timeout and always surfaces a clear, actionable error — but the *actual* bucket still doesn't exist; provisioning it requires one manual console click under the project-owner account, which I cannot perform myself (I don't have that Google account signed into this environment's browser, and there is no CLI/API path to provision it). Ticket submission without a photo is unaffected — nothing else depends on Storage.
- **Found and fixed post-deployment**: two Firestore queries (`Ticket` filtered by `tenantId` + ordered by `submittedAt`; `MonthlyReport` filtered by `providerId` + ordered by `generatedAt`) require composite indexes that weren't in `firestore.indexes.json` — the emulator doesn't enforce this, so it only surfaced against real production Firestore. Symptom: the tenant's "My Tickets" page and the provider's "Reports" page silently rendered empty lists (the affected `onSnapshot` calls have no error handler, so the underlying `failed-precondition` error was swallowed). Fixed by adding both composite indexes and deploying them (`firebase deploy --only firestore:indexes` — no rules, functions, or code changed); verified via direct production queries and the live UI after the index finished building. All other client queries were audited for the same pattern and are already covered by existing indexes.
- **Direct-URL authorization bypass — investigated and confirmed NOT a vulnerability.** A manual test suggested `/admin/stats` might be reachable unauthenticated in an Incognito window. Reproduced with a genuinely cleared session (verified via network capture: zero Firebase Auth/Firestore requests) — the route guard correctly rendered nothing (no admin content, no data) rather than the protected page. Independently confirmed by retesting in a freshly-opened Chrome Incognito window and a freshly-opened Edge InPrivate window (both with no prior login): both correctly redirected to the login page. The original observation was a residual authenticated session in a reused Incognito window, not a bypass. `(admin)`/`(provider)`/`(tenant)` route guards and Firestore Security Rules both correctly deny access to unauthenticated and wrong-role requests; no code change was made or needed.
